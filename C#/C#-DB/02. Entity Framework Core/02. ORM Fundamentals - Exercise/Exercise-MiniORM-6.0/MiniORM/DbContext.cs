...
public abstract class DbContext
{
    ...
    public void SaveChanges()
    {
        ...
        foreach (IEnumerable<object> dbSet in dbSets)
        {
            ...
            {
                throw new InvalidOperationException(String.Format(ExceptionMessages.InvalidEntitiesException, invalidEntities.Count(), dbSet.GetType().Name));
            }
        }
    ...
    private void MapAllRelations()
    {
        foreach (KeyValuePair<Type, PropertyInfo> dbSetInfo in this.dbSetProperties)
        {
            ...
            MethodInfo mapRelationsMethod = typeof(DbContext).GetMethod("MapRelations", BindingFlags.Instance | BindingFlags.NonPublic).MakeGenericMethod(dbSetEntityType);
        ...
        foreach (PropertyInfo fk in foreignKeys)
        {
            ...
            foreach (TEntity entity in dbSet)
            {
                ...
                object navigationPropertyValue = navigationDbSet
                    .First(cnp => navigationEntityPrimaryKey.GetValue(cnp).Equals(foreignKeyValue));
